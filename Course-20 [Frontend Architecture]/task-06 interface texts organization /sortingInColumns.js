...
/// SHOWCASE ON CODEPEN - https://codepen.io/yury-kurylovich/pen/wvdEbpb
...
const render = () => {
  ...
  let table = `<table class="table"><tbody><tr><th><a href="">Name (${state.name})</a></th>
<th><a href="">Value (${state.value})</a></th></tr>`;
  let tbl;
  if (
    state.name !== "Unsorted" ||
    (state.name === "Unsorted" && state.value === "Unsorted")
  ) {
    let keys = state.name === "Desc"
